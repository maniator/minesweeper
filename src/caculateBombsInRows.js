...
const generateRandomFromZero = (max) => Math.floor(Math.random() * (max - 1));
...
export const calculateBombsInRows = ({ rows, columns, bombs, noBomb = {} }) => {
    ...
    const noBombRow = noBomb.row || generateRandomFromZero(rows);
    const noBombColumn = noBomb.column || generateRandomFromZero(columns);
